	...
	//messageに順位が含まれている場合
		...
		// 各種パラメータを指定して接続
			...
			if( $pg_conn ) {
				...
				//順位表の作成　2017.08.21
				$sqlranksum = "select rank as "順位",sum(case player when '".$playerToday[0]."' then 1 else 0 end),sum(case player when '".$playerToday[1]."' then 1 else 0 end),sum(case player when '".$playerToday[2]."' then 1 else 0 end),sum(case player when '".$playerToday[3]."' then 1 else 0 end) from mjtable where date='".$date_s."' group by rank;";
				...
				$resRanksum = pg_query( $pg_conn, $sqlranksum); //2017.08.21
